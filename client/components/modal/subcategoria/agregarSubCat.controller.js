...
angular.module('pickngoApp')
    .controller('AgregarSubCatCtrl', function($scope, Auth, categorias, $http, Notification) {
        Auth.getCurrentUser();
        $scope.categorias = categorias;
        $scope.agregarSubCat = function(valid) {
            if (valid) {
                $http.post('api/subcategorias/', {idCat: $scope.subcategoria.categoria, nombre: $scope.subcategoria.nombre})
                    .then(resp => { 
