...
angular.module('pickngoApp')
    .controller('AgregarCategoriaCtrl', function($scope, Auth, departamentos, $http, Notification) {
        Auth.getCurrentUser();
        $scope.departamentos = departamentos;
        $scope.guardarCategoria = function(valid) {
            if (valid) {
                $http.post('api/categorias/', {idDepto: $scope.categoria.departamento, nombreCategoria: $scope.categoria.nombre})
                    .then(resp => {
